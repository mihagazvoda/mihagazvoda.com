...
This is meant as a concise guide to refresh your knowledge. 
...
* a well-behaved R object compatible with saveRDS()^[[Don't be like me.](https://stackoverflow.com/questions/61359114/how-to-use-shiny-app-as-a-target-in-drake)].


# Workflow
Even if you use `drake`, it makes sense to develop interactively. With `r_make("make.R")` you build your project. With `loadd` or `readd` you can return targets to your session - and then interactively use them. 
<!-- TODO output? -->
...
library(dplyr)
tibble::tibble(
  `function name` = c("vis_drake_graph()", "code_to_function()", "loadd()", "readd()"),
  description = c("Shows an interactive visual network representation of your workflow.", "Creates drake_plan()-ready functions from scripts like these.", "add description", "add description")
  ) %>% 
knitr::kable()
```
