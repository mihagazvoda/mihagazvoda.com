...
![Gaussian distribution with parameters $\mu$ and $\sigma$ describing heights of all people. Observed is only part of the distribution above the wall height $h_w$.](../../images/wall-puzzle/gaussian.png)

How to write this down?
We will start with [Bayes' theorem](https://en.wikipedia.org/wiki/Bayes%27_theorem#Statement_of_theorem). We are interested in posterior distribution $p(\mu, \sigma | D_{bias})$ of parameters $\mu$ and $\sigma$ that describe the height of people behind a wall given the observed heights $D_{bias}$. Since $p(D_{bias})$ serves as a normalizing constant that makes the posterior density integrate to one, we can throw it away.^[More about that on [Cross Validated](https://stats.stackexchange.com/questions/12112/normalizing-constant-in-bayes-theorem).] Priors of the parameters can be rewritten as their product. Likelihood is a product of the likelihoods of all observed heights. But beware, we are can't observe some of them.
...
The most likely parameters for Gaussian distribution describing people height standing behind a wall are $\mu \approx 185cm$  and $\sigma \approx 4.8cm$. Note that $\mu$ is smaller compared to average of observations $\bar{h} = 186.3cm$.   
...
ggplot(df, aes(x = mu, y = sigma, z = posterior_unstd)) +
  ...
  labs(
    ...
    subtitle = "Brighter colors present zones with higher posterior values."
  ) +
  theme_classic() +
...
<details>
<summary>Here are additional plots to better grasp the intuition about the posterior.</summary>

The plot below displays normalized probability densities for heights between 180 and 190cm. Normalized probability density is calculated the same as in $p(h_i|\mu, \sigma)$ above; that is divided by the portion of the Gaussian above the wall height. 
...
To get the posterior value for some $\mu$ and $\sigma$ you just have to multiply densities (`y` values on the plot). We can see that $\sigma$ between 10 and 15 make the more sense because higher $\sigma$ values are lower for all heights and for small $\sigma$ density for height 181cm is bigger but get's compensated because when multiplying with densities for 188 and 190 which are way smaller. 

</details> 

# Don't give up when your data is biased.

About another such example, getting customer complaints about their waiting time only when they've been waiting for too long, [talked Vincent on PyData](https://youtu.be/dE5j6NW-Kzg?t=721). He also explains the riddle I've solved without giving the the solution. 
