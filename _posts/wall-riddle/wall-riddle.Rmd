...
calc_likelihood <- function(mu, sigma, observations, wall) {
  exp(sum(
    dnorm(observations, mu, sigma, log = TRUE) -
      pnorm(wall, mu, sigma, lower.tail = FALSE, log.p = TRUE)
  ))
}
```
...
<details>
<summary>Have additional thoughts about the shape?</summary>
```{r}
...
tibble(
  ...
  theme_classic() + 
  labs(
    title = "Probability densities for Gaussians with mu = 170",
    x = "Height [cm]",
    y = "Normalized robability density"
  )
```
</details> 
...
About another such example, getting customer complaints only when they've been waiting for a long time, [talked Vincent on PyData](https://youtu.be/dE5j6NW-Kzg?t=721). He also explains the riddle I've solved^[It's borrowed from David Mackay's [Information Theory, Inference, and Learning Algorithms](http://www.inference.org.uk/mackay/itila/book.html)] but not the solution. 
