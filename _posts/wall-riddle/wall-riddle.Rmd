...
4. Calculate posterior as a product of likelihood and parameters' posteriors. Normalize the posterior to see what's the probability of occurring for such parameters.
...
<details>
<summary>Additional plots to get better understand the posterior.</summary>
...
To get the posterior value for some $\mu$ and $\sigma$ you just have to multiply densities (`y` values on the plot). We can see that $\sigma$ between 10 and 15 make the more sense because higher $\sigma$ values are lower for all heights and small $\sigma$ density for height 181cm is bigger but get's compensated because when multiplying with densities for 188 and 190 which are way smaller. 

</details> 

# Don't give up when your data is biased.

About another such example, getting customer complaints about their waiting time only when they've been waiting for longer than usual, [talked Vincent on PyData](https://youtu.be/dE5j6NW-Kzg?t=721). He also explains the riddle I've solved without giving the the solution. 
