...
With the `sample` function we draw, with replacement three coins that will be in our pocket.

```{r}
sample(coin_sample_space, size = 3, replace = TRUE)
```

With these three coins, we buy strings of appropriate lengths. To determine whether they can shape a triangle the sum of shorter two strings has to be longer than the longest one.
...
We repeat the same process zillion, ok `n`, times:
